...
namespace EnhancedMap.Core.MapObjects
{
    public class SharedLabelObject : RenderObject
    {
        ...
        public override bool Render(Graphics g, int x, int y, int canvasW, int canvasH)
        {
            ...
            Image img = Resources.SHAREDLABEL;
            ...
            if (buildX > -img.Width / 2 && buildX - img.Width / 2 < canvasW && buildY > -img.Height / 2 && buildY - img.Height / 2 < canvasH)
            {
                ...
                if (MouseManager.Location.X >= Position.X - wx && MouseManager.Location.X <= Position.X + wx && MouseManager.Location.Y >= Position.Y - wy && MouseManager.Location.Y <= Position.Y + wy && !string.IsNullOrEmpty(Description))
                {
